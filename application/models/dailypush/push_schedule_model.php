<?php
/**
 * Description of Push_schedule_model
 * 
 * @author  dhanyalvian@gmail.com
 */

class Push_schedule_model extends MY_Model {
	 ...
	 */
	$aColumns = array ('a.service', 'a.operator', 'a.sdc', 'a.recurring_type', 'a.content_label', 'a.handlerfile', 'DATE_FORMAT(a.push_time, \'' . $this->dateTimeFormatMysql . '\') push_time', 'FORMAT(a.price, 0) price', 'status', 'a.id');
	$aColumnsX = array ('service', 'operator', 'sdc', 'recurring_type', 'content_label', 'handlerfile', 'push_time', 'price', 'status', 'id');
        $aColumnsY = array ('a.service', 'a.operator', 'a.sdc', 'a.recurring_type', 'a.content_label', 'a.handlerfile', 'a.push_time', 'a.price', 'a.status', 'a.id');
    ...
    public function save() {
        ...
        $recurringType = $this->input->post('recurring_type', true);
        $handlerFile = $this->input->post('handler_file', true) == 'custom' ? $this->input->post('handler_file_custom', true) : $this->input->post('handler_file', true);
        $contentLabel = $this->input->post('content_label', true);
        $contentSelect = $this->input->post('content_select', true);
        $notes = $this->input->post('notes', true);
        $price = $this->input->post('price', true);
        $pushTime = $this->input->post('push_time', true);
        ...
        if ($this->input->post('action', true) == 'add') {
            $data = array (
                ...
                'notes' => $notes,
                'price' => $price,
                'push_time' => date('Y-m-d H:i:s', strtotime($pushTime)),
                'status' => $status,
                'created' => $dateCreated,
                'modified' => $dateModified
            );
            $this->dbPush->insert($this->tblPushSchedule, $data);
        }
        else {
            $data = array (
                'service' => $service,
            ...
            $this->dbPush->update($this->tblPushSchedule, $data);
        }
    ...
    public function edit($id) {
        $this->dbPush->select('id, service, operator, sdc, recurring_type, handlerfile, content_label, content_select, price, notes, status, DATE_FORMAT(push_time, \'' . $this->dateTimeFormatMysql . '\') push_time', false);
        $this->dbPush->where('id', $id);
        $this->dbPush->limit(1);
        $query = $this->dbPush->get($this->tblPushSchedule);
        $result = array ();
    ...
    public function delete($id) {
        $this->dbPush->where('id', $id);
        $this->dbPush->limit(1);
        $query = $this->dbPush->delete($this->tblPushSchedule);
...
/* End of file push_schedule_model.php */
/* Location: ./application/models/dailypush/push_schedule_model.php */
